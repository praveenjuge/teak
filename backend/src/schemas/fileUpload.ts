...
export const createCardWithFileSchema = z.object({
  ...
  data: z.string().optional().transform((str) => {
    if (!str) return {};
    try {
  ...
  metaInfo: z.string().optional().transform((str) => {
    if (!str) return {};
    try {
      return JSON.parse(str);
    } catch {
...
export type CreateCardWithFileRequest = z.infer<typeof createCardWithFileSchema>;
...
function buildArtifactKey(userId: string, jobId: string): string {
  const hashUserId = Array.from(new TextEncoder().encode(userId))
...
export const runExportArchive = internalAction({
  ...
  handler: async (ctx, { jobId, userId }) => {
    ...
    const canceledBefore = await ctx.runQuery(internalAny.dataExport.isCancelRequested, {
      jobId,
    });
    if (canceledBefore) {
    ...
    const canceledAfter = await ctx.runQuery(internalAny.dataExport.isCancelRequested, {
      jobId,
    });
    if (canceledAfter) {
      return { ok: false, failureClass: "canceled" };
...
export const deleteArtifact = internalAction({
  ...
  handler: async (_ctx, { artifactKey }) => {
    let config: ReturnType<typeof r2Config>;
    try {
      config = r2Config();
    } catch {

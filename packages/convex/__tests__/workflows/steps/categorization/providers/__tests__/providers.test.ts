...
describe("categorization providers common", () => {
    ...
    it("getRawAttribute", () => {
        ...
        const map2 = new Map([
            ["meta", { attributes: [{ name: "Content", value: "upper" }, { name: "other", value: "other" }] }]
        ]);
...
describe("enrichAmazon", () => {
    it("extracts price and currency", () => {
        ...
        expect(result.facts).toContainEqual({ label: "Price", value: "$10.00 USD" });
...
describe("enrichGithub", () => {
    it("extracts stats and language", () => {
        ...
        expect(result.facts).toContainEqual({ label: "Language", value: "TypeScript" });
        expect(result.facts).toContainEqual({ label: "Updated", value: "Oct 10, 2023" });
    });
...
describe("enrichGoodreads", () => {
    it("extracts rating and count", () => {
        const map = new Map([
            ["meta[property='books:rating:average']", { attributes: [{ name: "content", value: "4.2" }] }],
            ["meta[property='books:rating:count']", { attributes: [{ name: "content", value: "100" }] }]
        ]);
        const result = enrichGoodreads(map);
        expect(result.facts).toContainEqual({ label: "Average rating", value: "4.20 / 5" });
...
describe("enrichImdb", () => {
    it("extracts rating and votes", () => {
        const map = new Map([
            ...
            ["meta[name='imdb:votes']", { attributes: [{ name: "content", value: "100k" }] }]
        ]);
        const result = enrichImdb(map);
        expect(result.facts).toContainEqual({ label: "IMDb rating", value: "8.50 / 10" });
    ...
    it("extracts runtime and released", () => {
        const map = new Map([
            ...
            ["meta[property='video:release_date']", { attributes: [{ name: "content", value: "2023-12-21" }] }]
        ]);
        const result = enrichImdb(map);
        expect(result.facts).toContainEqual({ label: "Runtime", value: "2h 30m" });
        expect(result.facts).toContainEqual({ label: "Released", value: "Dec 21, 2023" });
    });
...
describe("enrichDribbble", () => {
    it("extracts stats and tags", () => {
        const map = new Map([
            ...
            ["meta[name='keywords']", { attributes: [{ name: "content", value: "design, ui, ux" }] }],
            ["meta[name='author']", { attributes: [{ name: "content", value: "Designer Name" }] }]
        ]);
        const result = enrichDribbble(map);
        expect(result.facts).toContainEqual({ label: "Likes", value: "50" });
        expect(result.facts).toContainEqual({ label: "Designer", value: "Designer Name" });
        expect(result.facts).toContainEqual({ label: "Tags", value: "design, ui, ux" });
    ...
    it("extracts stats from twitter meta", () => {
        const map = new Map([
            ...
            ["meta[name='twitter:data1']", { attributes: [{ name: "content", value: "100" }] }],
            ["meta[name='twitter:label2']", { attributes: [{ name: "content", value: "Views" }] }],
            ["meta[name='twitter:data2']", { attributes: [{ name: "content", value: "1k" }] }]
        ]);
        const result = enrichDribbble(map);
    ...
    it("extracts designer from og:title", () => {
        const map = new Map([
            ["meta[property='og:title']", { attributes: [{ name: "content", value: "Shot Title by John Doe on Dribbble" }] }]
        ]);
        const result = enrichDribbble(map);
        expect(result.facts).toContainEqual({ label: "Designer", value: "John Doe" });
    });

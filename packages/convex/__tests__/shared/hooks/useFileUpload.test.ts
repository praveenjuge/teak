// @ts-nocheck
import { describe, expect, test, mock, beforeEach, beforeAll, afterAll, spyOn } from "bun:test";
...
import { useFileUploadCore, setFileUploadSentryCaptureFunction, FileUploadDependencies } from '../../../client/hooks/useFileUpload.client';
...
describe("useFileUploadCore", () => {
    ...
    describe("initialization", () => {
        ...
        test("creates hook with state object", () => {
            ...
            expect(hook.state.error === null || typeof hook.state.error === "object").toBe(true);
        });
    ...
    describe("uploadFile - error handling", () => {
        ...
        test("calls onError callback on failure", async () => {
            const file = { size: 100, name: "test.png", type: "image/png" } as any;
            mockUploadAndCreateCard.mockResolvedValue({ success: false, error: "test error" });
        ...
        test("captures exception in Sentry on failure", async () => {
            ...
            expect(callArgs[1].tags).toEqual({ source: "convex", operation: "fileUpload" });
        });
    });
    ...
    describe("uploadFile - file type inference", () => {
        ...
        test("supports document type (word)", async () => {
            const file = { size: 100, name: "test.doc", type: "application/msword" } as any;
            mockUploadAndCreateCard.mockResolvedValue({ success: true, uploadUrl: "https://upload" });
            mockFetch.mockResolvedValue({ ok: true, json: async () => ({ storageId: "store_1" }) });
            mockFinalizeUploadedCard.mockResolvedValue({ success: true, cardId: "card_1" });
            await hook.uploadFile(file);

...
describe("R2 download URLs", () => {
  test("request public immutable browser caching for the signed URL lifetime", () => {
    const command = buildR2DownloadCommand("users/example/file.md", "bucket");

    expect(command.input).toMatchObject({
      Bucket: "bucket",
      Key: "users/example/file.md",
      ResponseCacheControl: "public, max-age=604800, stale-while-revalidate=86400, immutable",
    });
  });
});

...
import { initializeCardProcessingStateHandler, startCardProcessingWorkflowHandler, workflow } from "./manager";
...
describe("workflow manager", () => {
    describe("initializeCardProcessingState", () => {
        ...
        test("throws if card not found", async () => {
            ...
            expect(initializeCardProcessingStateHandler(ctx, { cardId: "c1" })).rejects.toThrow("Card c1 not found");
        });
        ...
        test("initializes processing state for text card", async () => {
            ...
            expect(mockDbPatch).toHaveBeenCalledWith("cards", "c1", expect.objectContaining({
                aiTags: undefined,
    ...
    describe("startCardProcessingWorkflow", () => {
        ...
        test("starts workflow and initializes state", async () => {
            ...
            const result = await startCardProcessingWorkflowHandler(ctx, { cardId: "c1" });

...
type HomeClientProps = {
  preloadedCards: Preloaded<typeof api.cards.searchCards>;
};

export default function HomeClient({ preloadedCards }: HomeClientProps) {
  ...
  const queryArgs = useMemo(
    () => ({
      ...
      limit: DEFAULT_QUERY_ARGS.limit,
    }),
    [filterTags, searchTerms, showFavoritesOnly, showTrashOnly]
  );

  const usingDefaultQuery =
    queryArgs.searchQuery === undefined &&
    queryArgs.types === undefined &&
    queryArgs.favoritesOnly === undefined &&
    queryArgs.showTrashOnly === undefined;

  const preloadedCardsResult = usePreloadedQuery(preloadedCards);
  const cardsFromQuery = useQuery(
    api.cards.searchCards,
    usingDefaultQuery ? "skip" : queryArgs
  );

  const cards = usingDefaultQuery ? preloadedCardsResult : cardsFromQuery;

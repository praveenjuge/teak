...
const saveCurrentPage = async () => {
  ...
  if (saveResult.status === "unauthenticated") {
    setState(
      "signed-out",
      "Sign in to Teak",
      saveResult.message || "Sign in to save pages."
    );
    return;
  }
  ...
  setState(
    "error",
    "Save failed",
    saveResult.message || "Teak could not save this page."
  );
};
...
const startSignIn = async () => {
  ...
  setState(
    "error",
    "Sign in failed",
    response.message || "Teak could not start sign in."
  );
};
  ...
  try {
    ...
    setState(
      ...
      error instanceof Error
        ? error.message
        : "Try again from a regular website."
    );
  }
};

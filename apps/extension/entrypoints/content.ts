import {
  type AuthStateResponse,
  MESSAGE_TYPES,
  type TeakSaveResponse,
} from "../types/messages";
import {
  type ExtractedPost,
  getInlineSaveButtonVariant,
  isPlatformInlineSaveHost,
  type Platform,
} from "../types/social";
import {
  type ButtonState,
  mapSaveResponseToButtonState,
  shouldAllowClick,
} from "./content/buttonStateMachine";
import { getInlineSaveButtonPosition } from "./content/platformButtonLayout";
import {
  extractHackerNewsPost,
  findHackerNewsPosts,
} from "./content/platforms/hackernews";
import {
  extractHeyDesignerPost,
  findHeyDesignerPosts,
} from "./content/platforms/heydesigner";
import {
  extractInstagramPost,
  findInstagramPosts,
} from "./content/platforms/instagram";
import {
  extractPinterestPost,
  findPinterestPosts,
} from "./content/platforms/pinterest";
import {
  extractSidebarPost,
  findSidebarPosts,
} from "./content/platforms/sidebar";
import {
  extractWebDesignerNewsPost,
  findWebDesignerNewsPosts,
} from "./content/platforms/webdesignernews";
import { extractXPost, findXPosts } from "./content/platforms/x";

export default defineContentScript({
  matches: ["<all_urls>"],
  main() {
    const host = window.location.hostname.toLowerCase();
    const platformBinding = getPlatformBinding(host);
    if (!platformBinding) {
      return;
    }

    injectStyleTag();

    const savedPostKeys = new Set<string>();
    let isAuthenticated = false;
    let authPollTimer: number | undefined;
    let scanTimer: number | undefined;

    const scheduleScan = () => {
      if (scanTimer || !isAuthenticated) {
        return;
      }

      scanTimer = window.setTimeout(() => {
        scanTimer = undefined;
        scanAndInjectButtons({
          platformBinding,
          savedPostKeys,
        });
      }, 120);
    };

    const cleanupButtons = () => {
      const buttons =
        document.querySelectorAll<HTMLButtonElement>(".teak-save-button");
      for (const button of buttons) {
        button.remove();
      }

      const boundPosts = document.querySelectorAll<HTMLElement>(
        "[data-teak-save-bound='1']"
      );
      for (const post of boundPosts) {
        post.removeAttribute("data-teak-save-bound");

        if (post.dataset.teakPositionManaged === "1") {
          post.style.position = post.dataset.teakOriginalPosition ?? "";
          delete post.dataset.teakOriginalPosition;
          delete post.dataset.teakPositionManaged;
        }
      }
    };

    const refreshAuthState = async () => {
      try {
        const response = await sendRuntimeMessage<AuthStateResponse>({
          type: MESSAGE_TYPES.GET_AUTH_STATE,
        });

        const nextAuthState = response.authenticated === true;
        if (nextAuthState === isAuthenticated) {
          return;
        }

        isAuthenticated = nextAuthState;
        if (!isAuthenticated) {
          cleanupButtons();
          return;
        }

        scheduleScan();
      } catch {
        isAuthenticated = false;
        cleanupButtons();
      }
    };

    const observer = new MutationObserver(() => {
      scheduleScan();
    });

    observer.observe(document.body, {
      childList: true,
      subtree: true,
    });

    authPollTimer = window.setInterval(() => {
      void refreshAuthState();
    }, 15_000);

    window.addEventListener("beforeunload", () => {
      if (authPollTimer) {
        clearInterval(authPollTimer);
      }
      if (scanTimer) {
        clearTimeout(scanTimer);
      }
      observer.disconnect();
    });

    void refreshAuthState();
  },
});

type PlatformBinding = {
  bindHoverScope?: (postElement: HTMLElement) => void;
  extractPost: (postElement: HTMLElement) => ExtractedPost | null;
  findPosts: (root: ParentNode) => HTMLElement[];
  hostMatches: (host: string) => boolean;
  mountButton?: (
    postElement: HTMLElement,
    button: HTMLButtonElement
  ) => boolean;
  platform: Platform;
};

const PLATFORM_BINDINGS: PlatformBinding[] = [
  {
    platform: "x",
    hostMatches: (host) => isPlatformInlineSaveHost("x", host),
    findPosts: findXPosts,
    extractPost: extractXPost,
  },
  {
    platform: "hackernews",
    hostMatches: (host) => isPlatformInlineSaveHost("hackernews", host),
    findPosts: findHackerNewsPosts,
    extractPost: extractHackerNewsPost,
    mountButton: (postElement, button) => {
      const titleLine = postElement.querySelector<HTMLElement>(".titleline");
      if (!titleLine) {
        return false;
      }

      titleLine.append(button);
      return true;
    },
    bindHoverScope: bindHackerNewsHoverState,
  },
  {
    platform: "instagram",
    hostMatches: (host) => isPlatformInlineSaveHost("instagram", host),
    findPosts: findInstagramPosts,
    extractPost: extractInstagramPost,
  },
  {
    platform: "pinterest",
    hostMatches: (host) => isPlatformInlineSaveHost("pinterest", host),
    findPosts: findPinterestPosts,
    extractPost: extractPinterestPost,
  },
  {
    platform: "sidebar",
    hostMatches: (host) => isPlatformInlineSaveHost("sidebar", host),
    findPosts: findSidebarPosts,
    extractPost: extractSidebarPost,
    mountButton: (postElement, button) => {
      const title = postElement.querySelector<HTMLElement>(".post-title");
      if (!title) {
        return false;
      }

      title.append(button);
      return true;
    },
  },
  {
    platform: "webdesignernews",
    hostMatches: (host) => isPlatformInlineSaveHost("webdesignernews", host),
    findPosts: findWebDesignerNewsPosts,
    extractPost: extractWebDesignerNewsPost,
    mountButton: (postElement, button) => {
      const title = postElement.querySelector<HTMLElement>(
        ".single-post-text h3"
      );
      if (!title) {
        return false;
      }

      title.append(button);
      return true;
    },
  },
  {
    platform: "heydesigner",
    hostMatches: (host) => isPlatformInlineSaveHost("heydesigner", host),
    findPosts: findHeyDesignerPosts,
    extractPost: extractHeyDesignerPost,
    mountButton: (postElement, button) => (postElement.append(button), true),
  },
];

const ICONS: Record<Exclude<ButtonState, "idle">, string> = {
  saving: `<svg class="teak-spin" viewBox="0 0 24 24" aria-hidden="true"><path d="M21 12a9 9 0 1 1-6.219-8.56" /></svg>`,
  saved: `<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M5 13l4 4L19 7"/></svg>`,
  duplicate: `<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M6 12l4 4L18 8"/><path d="M4 4h10a1 1 0 0 1 1 1v3" /></svg>`,
  error: `<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M7 7l10 10M17 7L7 17"/></svg>`,
};

const STATE_LABELS: Record<ButtonState, string> = {
  idle: "Save to Teak",
  saving: "Saving to Teak",
  saved: "Saved to Teak",
  duplicate: "Already saved in Teak",
  error: "Save failed. Retry",
};

const STYLE_ID = "teak-inline-save-style";
// biome-ignore lint/complexity/noUselessStringRaw: <cause i want it>
const TEAK_INLINE_SVG_MARKUP = String.raw`<svg width="500" height="500" viewBox="0 0 500 500" fill="#fff" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" clip-rule="evenodd" d="M49.1704 331.718C47.5419 324.559 46.7257 317.27 46.7257 309.84C46.7257 306.971 46.851 304.101 47.101 301.23C47.1201 301.009 47.1388 300.798 47.1581 300.591L47.276 299.417C47.5233 297.664 47.8762 296.116 48.3101 294.751C50.979 293.641 54.1485 292.437 57.8451 291.143L57.9081 291.121L57.9699 291.099C61.2193 289.91 64.8199 288.775 68.7816 287.703C70.7614 287.168 72.8274 286.647 74.9809 286.145L75.0186 286.137L76.3124 285.84L76.4085 285.818C82.0464 284.46 88.1402 283.26 94.6961 282.218C100.502 285.863 106.757 288.789 113.407 291.015L113.469 291.038L113.61 291.084C125.271 294.913 137.746 296.691 150.835 296.691C163.914 296.691 176.272 295.157 187.539 291.679C192.729 294.338 197.291 297.384 201.26 300.795L201.293 300.822L201.325 300.85C208.951 307.285 214.683 315.256 218.547 324.917L218.555 324.935L218.562 324.955C222.456 334.567 224.503 345.7 224.503 358.505V399.091L224.515 400.917V400.955L224.516 400.989C224.544 402.983 224.616 404.919 224.733 406.8L224.818 408.046C224.91 409.283 225.023 410.497 225.157 411.688C225.168 411.784 225.178 411.883 225.189 411.978C225.213 412.188 225.239 412.409 225.267 412.633C225.842 417.316 226.763 421.681 228.09 425.663C230.309 432.319 234.191 437.943 239.62 442.362V445.063C239.62 452.437 238.718 458.727 237.065 464.04C236.586 465.58 236.044 467.036 235.443 468.416L234.886 469.658C232.119 475.627 228.681 480.392 224.646 484.123L223.809 484.875C222.754 485.8 221.667 486.662 220.546 487.462C217.327 489.764 213.798 491.588 209.931 492.934L208.77 493.324C203.343 495.082 197.847 495.952 192.251 495.952C191.722 495.952 191.194 495.941 190.666 495.924C190.504 495.92 190.34 495.913 190.176 495.906C190.098 495.902 190.02 495.899 189.941 495.895C188.983 495.851 188.028 495.783 187.078 495.683C184.666 495.43 182.268 494.997 179.882 494.384L179.788 494.359L179.695 494.338C175.849 493.433 172.168 492.021 168.634 490.087C165.393 488.241 162.376 485.897 159.58 483.013C157.113 480.203 155.096 476.875 153.565 472.936L151.241 466.962H144.831C140.859 466.962 136.347 468.042 132.819 471.569L132.625 471.765L132.442 471.971C129.464 475.321 128.417 479.387 128.417 483.166C128.417 486.547 129.344 489.697 130.727 492.527C129.107 493.012 127.321 493.463 125.357 493.863C118.528 495.254 111.562 495.952 104.449 495.952L103.356 495.948C102.392 495.94 101.436 495.919 100.489 495.888C98.6705 495.83 96.883 495.732 95.1274 495.594C87.1226 494.963 79.7704 493.501 73.0361 491.256C63.9721 488.188 56.2956 483.716 49.8716 477.889C43.6195 472.093 38.6635 465.027 35.0251 456.564C31.7978 448.632 29.9824 439.362 29.7673 428.603L29.7513 427.521L29.7456 426.427C29.7456 418.859 30.5182 412.127 31.9821 406.177C32.7834 402.919 33.7913 399.898 34.993 397.1L35.3454 396.297C39.2323 387.816 44.3404 381.069 50.6026 375.85L50.6415 375.817L50.6793 375.784C57.1125 370.304 64.5932 366.211 73.226 363.532C82.2243 360.782 91.7896 359.385 101.964 359.384C106.687 359.384 110.762 359.582 114.219 359.952L114.384 359.97L114.55 359.982C118.202 360.243 121.224 360.56 123.655 360.921C125.501 361.243 127.141 361.558 128.577 361.868C129.161 361.993 129.706 362.117 130.213 362.237L130.745 362.366L130.96 362.417L131.014 362.431L139.655 364.537L142.209 356.019C142.77 354.15 143.226 352.014 143.226 349.807C143.226 343.928 139.786 339.786 136.022 337.367C132.878 335.261 129.229 334.052 125.542 333.368C121.986 332.575 118.051 332.052 113.791 331.758C109.567 331.468 105.279 331.323 100.929 331.323C90.8459 331.323 80.7816 332.437 70.747 334.649C64.0776 336.121 57.7826 338.122 51.8851 340.672C50.9059 338.035 49.994 335.059 49.1704 331.718ZM150.888 409.238V399.867H126.399C121.79 399.867 116.239 401.076 112.366 405.724L112.34 405.756L112.312 405.789C109.62 409.08 108.331 413.003 108.331 417.108C108.331 422.286 110.361 427.848 115.951 431.042L116.07 431.11L116.192 431.174C119.175 432.765 122.396 433.606 125.691 433.855C125.709 433.856 125.726 433.857 125.744 433.858C125.749 433.858 125.755 433.859 125.76 433.859C125.768 433.861 125.776 433.861 125.784 433.861C125.823 433.864 125.867 433.866 125.903 433.869C125.909 433.87 125.916 433.87 125.922 433.87C125.928 433.871 125.934 433.871 125.941 433.871C126.023 433.877 126.105 433.882 126.188 433.886C126.204 433.888 126.22 433.889 126.237 433.89C126.245 433.89 126.253 433.891 126.261 433.891C126.29 433.892 126.319 433.895 126.348 433.896C126.491 433.903 126.634 433.909 126.776 433.914L126.797 433.916L126.871 433.917L126.886 433.918C126.957 433.92 127.032 433.923 127.106 433.924L127.849 433.933C133.757 433.933 139.776 432.443 144.419 428.073C149.688 423.114 150.888 415.717 150.888 409.238Z" />
<path d="M175.426 166.922V126.282H209.175C209.288 126.074 209.401 125.865 209.511 125.654L209.552 125.576L209.661 125.365L209.781 125.131L209.811 125.071L209.858 124.979C210.118 124.462 210.371 123.936 210.617 123.401C212.06 120.255 213.268 116.768 214.213 112.918L214.24 112.805C214.709 110.995 215.118 109.222 215.466 107.487C216.324 103.214 216.825 99.1736 216.989 95.3573C217.012 94.8123 217.028 94.272 217.038 93.7363L217.037 93.8208C217.045 93.4045 217.048 92.9912 217.048 92.5809C217.048 86.0053 216.137 79.3929 214.285 72.7236L214.257 72.6264L214.232 72.5268C212.984 67.6227 211.26 63.1539 209.087 59.0863H169.213V6.69652C165.936 6.10963 162.818 5.62424 159.854 5.24484L159.82 5.24026L157.597 4.97256L157.621 4.97486C152.043 4.32611 146.468 4.00248 140.896 4.00248C133.644 4.00248 126.541 4.39786 119.585 5.18535C115.088 5.69439 111.274 6.24482 108.105 6.8235C107.8 7.75128 107.484 8.72896 107.154 9.75561L107.081 9.98322L107.062 10.0416L107.02 10.1697C106.969 10.3296 106.918 10.4902 106.867 10.6518L106.773 10.9453C106.509 11.7746 106.236 12.6332 105.956 13.5215L105.935 13.5936L105.923 13.6302L105.911 13.668C104.391 18.3641 102.803 23.2673 101.146 28.3772C100.738 29.6343 100.33 30.8792 99.922 32.1123L99.9162 32.1306L99.9088 32.1537C99.903 32.1722 99.8952 32.1969 99.8865 32.2233C98.8358 35.3973 97.7828 38.4925 96.7303 41.5078C96.5198 42.1108 96.3094 42.7106 96.0987 43.3074C94.4079 48.098 92.8816 52.1073 91.5263 55.27L90.5402 57.5728L88.536 59.0759C84.0463 62.4431 78.8775 66.2141 73.0361 70.3865C72.3304 70.9024 71.6269 71.4128 70.9255 71.9159C66.9126 74.7945 62.8604 77.5408 58.7719 80.1584C58.3665 82.4516 57.9927 85.152 57.661 88.2852C57.2688 93.1548 57.0811 97.4091 57.0811 101.07C57.0811 106.895 57.4023 112.646 58.0432 118.326C58.4466 121.196 58.9339 123.916 59.4995 126.489H105.406L107.542 132.897C108.119 134.629 108.437 136.457 108.437 138.346C108.437 138.632 108.43 138.92 108.415 139.209C108.407 139.354 108.398 139.499 108.386 139.644L108.346 140.083C107.878 144.53 105.627 148.245 102.203 150.813C98.1114 153.882 92.9977 154.551 88.711 154.551H85.2435V205.027V205.009L85.2481 206.151L85.2493 206.17C85.2525 206.52 85.2568 206.87 85.263 207.219C85.458 218.079 87.1083 227.204 89.9933 234.768L90.2896 235.52C93.3953 243.238 97.6516 249.509 102.99 254.515C103.93 255.396 104.898 256.238 105.895 257.042C110.462 260.726 115.693 263.659 121.655 265.816C122.146 265.994 122.643 266.166 123.145 266.332C131.314 269.012 140.516 270.41 150.835 270.41C156.387 270.41 161.518 270.075 166.243 269.424C167.749 269.217 169.212 268.978 170.634 268.707C175.878 267.71 180.543 266.291 184.67 264.499C185.759 264.026 186.812 263.527 187.829 263.002C188.191 262.815 188.55 262.624 188.905 262.429L188.974 262.391C189.188 262.273 189.4 262.154 189.611 262.034C190.513 261.519 191.386 260.983 192.229 260.424C192.383 260.322 192.535 260.22 192.685 260.118L192.972 259.921L193.675 259.426C194.371 258.924 195.046 258.404 195.704 257.869L196.219 257.442L196.169 257.486C199.463 254.721 202.162 251.568 204.309 248.008C205.883 245.395 207.191 242.517 208.211 239.345C208.463 238.564 208.697 237.762 208.915 236.942C208.991 236.651 209.066 236.358 209.138 236.066L209.313 235.325C209.714 233.583 210.039 231.758 210.285 229.846L210.288 229.808C210.372 229.151 210.447 228.482 210.513 227.805L210.529 227.627L210.532 227.595C210.535 227.56 210.539 227.526 210.541 227.491C210.543 227.47 210.546 227.45 210.547 227.43L210.553 227.361C210.699 225.722 210.791 224.025 210.822 222.267V222.219L210.828 221.878L210.829 221.809V221.738C210.83 221.682 210.832 221.627 210.832 221.567V221.613L210.836 220.764C210.836 216.717 210.42 212.657 209.579 208.573L209.552 208.442L209.529 208.31C209.195 206.418 208.838 204.583 208.459 202.8H206.955C198.085 202.8 189.926 200.221 183.853 193.933L183.743 193.818C177.461 187.087 175.426 177.314 175.426 166.922Z" /><path fill-rule="evenodd" clip-rule="evenodd" d="M228.212 163.196C228.212 180.695 230.951 196.127 236.245 209.633C238.834 216.071 241.901 222.007 245.432 227.46C248.598 232.344 252.147 236.857 256.082 241.005C257.053 242.029 258.048 243.03 259.067 244.011C281.115 264.623 312.047 272.77 341.62 273.095H341.575C371.252 273.217 396.616 274.604 426.349 255.004C427.119 250.327 427.319 242.69 427.319 237.951C427.319 235.311 427.089 232.325 426.593 228.971L426.563 228.784L426.545 228.596C426.282 226.15 425.897 223.786 425.391 221.5C425.282 221.006 425.166 220.513 425.044 220.02L425.069 220.111C424.964 219.687 424.855 219.267 424.743 218.851C424.123 216.552 423.423 214.484 422.658 212.629C421.655 213.166 420.619 213.702 419.552 214.236C414.871 216.577 409.494 218.682 403.491 220.586L403.351 220.629C397.409 222.412 390.762 223.938 383.443 225.23L383.267 225.26C375.661 226.452 367.366 227.03 358.414 227.03C357.657 227.03 356.902 227.021 356.146 227.009C350.202 226.904 344.264 226.39 338.335 225.468L338.236 225.453L338.136 225.434C331.62 224.276 325.695 222.877 320.401 221.213L320.285 221.178L320.17 221.137C314.906 219.332 310.235 217.252 306.375 214.795L306.25 214.716L306.127 214.632C302.472 212.14 297.479 207.624 297.479 200.677C297.479 199.748 297.545 198.833 297.655 197.946C297.677 197.767 297.702 197.588 297.728 197.41C297.755 197.225 297.783 197.041 297.813 196.858L297.9 196.336L298.046 195.824C298.781 193.254 300.24 190.733 302.652 188.803L307.741 184.733L313.329 188.085C315.252 189.238 317.35 190.327 319.634 191.345C323.589 193.107 328.082 194.646 333.144 195.934C336.205 196.699 339.304 197.32 342.441 197.795C347.58 198.574 352.834 198.968 358.207 198.968C369.938 198.968 380.665 197.664 390.434 195.127C400.068 192.625 408.121 188.882 414.761 184.027C421.251 179.147 426.285 173.021 429.914 165.529L429.938 165.477L429.965 165.424C432.688 159.978 434.496 153.527 435.232 145.921C435.327 144.935 435.404 143.928 435.463 142.9L435.474 142.706L435.479 142.607C435.485 142.484 435.492 142.363 435.498 142.246C435.501 142.17 435.505 142.095 435.508 142.021L435.549 141.022C435.561 140.686 435.57 140.348 435.579 140.008C435.586 139.669 435.593 139.323 435.596 138.972L435.597 138.85L435.599 138.803V138.707L435.603 137.879L435.602 137.931C435.602 126.223 433.306 115.31 428.753 105.094C424.191 94.8618 417.666 85.9208 409.097 78.2215L409.047 78.1758C400.673 70.5408 390.385 64.4439 378.012 59.9797L377.932 59.95C365.745 55.4268 351.826 53.0803 336.049 53.0803C321.345 53.0804 307.442 55.4636 294.281 60.1913L294.245 60.2039C291.995 61.0135 289.794 61.8893 287.646 62.8305L287.482 62.9026L287.437 62.9208C287.191 63.0294 286.942 63.1402 286.693 63.2526C286.041 63.5459 285.396 63.846 284.756 64.1506C281.717 65.5976 278.779 67.1821 275.944 68.905C270.26 72.3598 264.964 76.3805 260.051 80.9763C250.459 90.0772 242.678 101.462 236.783 115.304L236.745 115.288C236.387 116.948 235.994 118.612 235.566 120.281C234.405 124.986 232.918 129.532 231.077 133.883C230.008 138.934 229.234 144.242 228.762 149.815C228.398 154.118 228.212 158.578 228.212 163.196ZM352.503 120.476C349.082 118.521 345.32 117.69 341.564 117.594C341.314 117.587 341.064 117.585 340.812 117.585C335.07 117.585 329.503 119.281 325.084 123.433C324.843 123.659 324.6 123.896 324.368 124.134C320.782 127.824 318.842 132.443 318.09 137.334L317.131 143.557L322.531 146.796C324.565 148.016 326.926 149.074 329.445 150.018L329.69 150.112L329.941 150.189C331.415 150.65 332.932 150.986 334.472 151.219C336.011 151.452 337.595 151.585 339.209 151.631L339.28 151.634H339.349L339.368 151.635L339.394 151.634C339.796 151.644 340.199 151.651 340.606 151.651C345.014 151.651 350.173 150.82 354.267 147.408L354.293 147.385L354.321 147.363C356.148 145.816 357.708 143.891 358.786 141.568C359.728 139.539 360.205 137.416 360.308 135.303L360.325 134.873C360.325 134.838 360.326 134.803 360.327 134.767C360.327 134.736 360.328 134.704 360.328 134.671C360.329 134.63 360.329 134.589 360.33 134.547V134.41C360.33 129.003 358.052 123.553 352.503 120.476Z" /><path d="M332.623 421.903V428.032C332.623 433.369 333.173 438.854 334.236 444.476C335.266 449.921 336.59 455.292 338.209 460.589L338.255 460.74L338.306 460.89C340.084 466.076 342.162 471.12 344.538 476.021C345.616 478.245 346.736 480.384 347.897 482.432C343.737 485.609 338.724 488.404 332.758 490.756L332.737 490.764L332.716 490.774C324.612 494.015 315.134 495.788 304.132 495.896L303.063 495.901L302.042 495.892C297.069 495.8 291.911 495.039 286.551 493.559C281.193 492.043 276.371 489.802 272.029 486.85C267.971 483.868 264.64 480.258 261.995 475.971C260.629 473.683 259.624 471.096 259.026 468.139C260.809 460.772 261.61 453.017 261.61 445.011V441.491C261.61 439.608 261.545 437.593 261.338 435.591L261.241 434.736L259.991 424.73L258.387 423.737V277.533C266.468 282.49 275.176 286.633 284.462 289.984L284.506 290L284.644 290.052L284.782 290.097C302.66 296.365 322.158 299.332 343.029 299.332C343.838 299.332 344.64 299.327 345.439 299.321C343.927 306.311 342.107 312.783 339.995 318.752C339.4 319.562 338.504 320.648 337.214 322.031C335.21 324.177 333.211 326.392 331.214 328.674L331.102 328.803L330.994 328.935C329.039 331.341 327.271 333.718 325.706 336.066C324.015 338.603 322.062 342.244 322.062 346.441C322.062 352.576 325.663 356.916 329.594 359.527C329.736 359.621 329.881 359.716 330.03 359.809C330.106 359.857 330.182 359.906 330.259 359.954L330.375 360.024L330.387 360.031L330.413 360.046L330.428 360.055L330.466 360.079L330.482 360.088C330.751 360.25 331.02 360.403 331.286 360.55C331.586 360.714 331.894 360.877 332.211 361.036L338.014 363.938L342.774 359.527C348.759 353.979 354.017 348.22 358.513 342.243C363.143 336.268 367.239 329.859 370.804 323.026L370.823 322.991L370.843 322.955C373.749 317.263 376.34 311.255 378.626 304.937L379.079 303.668C379.154 303.456 379.228 303.243 379.302 303.032L379.343 302.913L379.433 302.655C379.484 302.506 379.536 302.359 379.586 302.21L379.791 301.606C379.845 301.447 379.9 301.288 379.953 301.13L379.986 301.03L380.051 300.837C380.567 299.343 381.075 297.822 381.579 296.272C391.283 294.641 400.241 292.48 408.325 289.682L408.354 289.672L408.382 289.662L408.477 289.629L408.573 289.594L408.666 289.559C416.183 286.811 423.158 283.599 429.444 279.831C430.769 280.146 432.06 280.49 433.316 280.868L433.404 280.896L433.493 280.919C440.838 282.966 446.708 286.001 451.339 289.86L451.389 289.902L451.442 289.944C456.062 293.683 459.661 298.359 462.257 304.115L462.27 304.146L462.285 304.178C464.788 309.622 466.185 316.23 466.19 324.232L466.173 325.736C466.019 332.617 464.773 338.705 462.551 344.105L462.055 345.275C459.615 350.854 456.354 355.905 452.246 360.456L452.236 360.465C447.894 365.291 442.796 369.555 436.894 373.249L435.819 373.889C430.087 377.252 424.009 380.067 417.575 382.33L398.631 388.996L415.908 399.233C419.885 401.59 423.827 403.675 427.733 405.478L427.87 405.541L428.009 405.6C431.826 407.214 435.714 408.682 439.671 410.001C443.695 411.342 447.909 412.523 452.31 413.55L452.343 413.558L452.375 413.564C455.548 414.281 458.946 414.955 462.562 415.589C463.646 417.82 464.613 420.058 465.458 422.301C466.438 424.901 467.261 427.514 467.93 430.139L467.951 430.222L467.975 430.304C469.702 436.601 470.539 442.669 470.539 448.532C470.539 455.239 469.465 461.478 467.365 467.309L467.337 467.387L467.31 467.466C465.394 473.101 462.432 478.008 458.4 482.265C454.624 486.258 449.671 489.616 443.305 492.218L442.75 492.435C440.284 493.371 437.591 494.132 434.655 494.7C430.599 495.486 426.09 495.901 421.1 495.901C412.71 495.901 405.271 494.297 398.638 491.226C392.028 487.963 386.171 483.355 381.049 477.281C375.918 471.194 371.591 463.62 368.179 454.392C364.772 445.181 362.438 434.662 361.26 422.76L359.992 409.928L332.623 421.903Z" /></svg>`;

const getPlatformBinding = (host: string): PlatformBinding | null =>
  PLATFORM_BINDINGS.find((binding) => binding.hostMatches(host)) ?? null;

const injectStyleTag = () => {
  if (document.getElementById(STYLE_ID)) {
    return;
  }

  const style = document.createElement("style");
  style.id = STYLE_ID;
  style.textContent = `
    .teak-save-button {
      border: 0;
      align-items: center;
      color: #fff;
      cursor: pointer;
      display: inline-flex;
      height: 30px;
      justify-content: center;
      position: absolute;
      top: var(--teak-save-button-top, 24px);
      right: var(--teak-save-button-right, 8px);
      bottom: var(--teak-save-button-bottom, auto);
      left: var(--teak-save-button-left, auto);
      transition: transform 0.15s ease, background-color 0.15s ease;
      width: 30px;
      z-index: 2147483000;
      background-color: oklch(57.7% 0.245 27.325);
      border-radius: 8px;
    }

    @media (hover: hover) {
      [data-teak-save-bound='1'] > .teak-save-button[data-state="idle"] {
        opacity: 0;
        pointer-events: none;
        transition:
          opacity 0.15s ease,
          transform 0.15s ease,
          background-color 0.15s ease;
      }

      [data-teak-save-bound='1']:hover > .teak-save-button[data-state="idle"],
      [data-teak-save-bound='1']:focus-within > .teak-save-button[data-state="idle"],
      [data-teak-save-bound='1'] > .teak-save-button[data-state="idle"]:focus-visible {
        opacity: 1;
        pointer-events: auto;
      }
    }

    .teak-save-button[data-state="idle"]:hover {
      transform: translateY(-1px);
    }

    .teak-save-button .teak-brand-icon {
      height: 18px;
      width: 18px;
      background-color: transparent;
      padding: 0;
      margin: 0;
      top: 0;
    }

    .teak-save-button .teak-brand-icon svg {
      display: block;
      height: 18px;
      transition: filter 0.15s ease;
      fill: #fff;
      width: 18px;
    }

    .teak-save-button[data-state="saving"] {
      cursor: wait;
    }

    .teak-save-button[data-state="saved"],
    .teak-save-button[data-state="duplicate"] {
      cursor: default;
    }

    .teak-save-button > svg {
      fill: none;
      height: 18px;
      stroke: currentColor;
      stroke-linecap: round;
      stroke-linejoin: round;
      stroke-width: 2;
      width: 18px;
      flex-shrink: 0;
    }

    .teak-save-button .teak-spin {
      animation: teak-spin 0.9s linear infinite;
    }

    .teak-save-button--compact {
      position: static;
      height: 18px;
      width: 18px;
      min-height: 18px;
      min-width: 18px;
      margin-left: 6px;
      border-radius: 5px;
      vertical-align: middle;
      flex: 0 0 auto;
    }

    .teak-save-button--compact[data-state="idle"] {
      opacity: 0;
      pointer-events: none;
      transition:
        opacity 0.15s ease,
        transform 0.15s ease,
        background-color 0.15s ease;
    }

    [data-teak-save-bound='1']:hover .teak-save-button--compact[data-state="idle"],
    [data-teak-save-bound='1'].teak-hn-hovering .teak-save-button--compact[data-state="idle"],
    [data-teak-save-bound='1']:focus-within .teak-save-button--compact[data-state="idle"],
    .teak-save-button--compact[data-state="idle"]:focus-visible {
      opacity: 1;
      pointer-events: auto;
    }

    .teak-save-button--compact .teak-brand-icon,
    .teak-save-button--compact .teak-brand-icon svg {
      height: 12px;
      width: 12px;
    }

    .teak-save-button--compact > svg {
      height: 12px;
      width: 12px;
    }

    @keyframes teak-spin {
      from { transform: rotate(0deg); }
      to { transform: rotate(360deg); }
    }
  `;

  document.documentElement.append(style);
};

const getIconMarkup = (state: ButtonState): string => {
  if (state === "idle") {
    return `<span class="teak-brand-icon" aria-hidden="true">${TEAK_INLINE_SVG_MARKUP}</span>`;
  }

  return ICONS[state];
};

const setButtonState = (
  button: HTMLButtonElement,
  state: ButtonState,
  errorMessage?: string
) => {
  button.dataset.state = state;
  button.innerHTML = getIconMarkup(state);
  const label =
    state === "error" && errorMessage
      ? `${STATE_LABELS[state]}: ${errorMessage}`
      : STATE_LABELS[state];
  button.setAttribute("aria-label", label);
  button.title = label;
  button.disabled =
    state === "saving" || state === "saved" || state === "duplicate";
};

const sendRuntimeMessage = <TResponse>(message: unknown): Promise<TResponse> =>
  new Promise((resolve, reject) => {
    chrome.runtime.sendMessage(message, (response) => {
      const runtimeError = chrome.runtime.lastError;
      if (runtimeError) {
        reject(new Error(runtimeError.message));
        return;
      }
      resolve(response as TResponse);
    });
  });

const createSaveButton = (
  extractedPost: ExtractedPost,
  savedPostKeys: Set<string>
): HTMLButtonElement => {
  const button = document.createElement("button");
  button.type = "button";
  button.className = "teak-save-button";
  button.dataset.postKey = extractedPost.postKey;
  button.dataset.permalink = extractedPost.permalink;
  button.dataset.platform = extractedPost.platform;
  const buttonVariant = getInlineSaveButtonVariant(extractedPost.platform);
  if (buttonVariant === "compact") {
    button.classList.add("teak-save-button--compact");
  } else {
    const buttonPosition = getInlineSaveButtonPosition(extractedPost.platform);
    button.style.setProperty("--teak-save-button-top", buttonPosition.top);
    button.style.setProperty("--teak-save-button-right", buttonPosition.right);
    button.style.setProperty(
      "--teak-save-button-bottom",
      buttonPosition.bottom
    );
    button.style.setProperty("--teak-save-button-left", buttonPosition.left);
  }

  if (savedPostKeys.has(extractedPost.postKey)) {
    setButtonState(button, "saved");
  } else {
    setButtonState(button, "idle");
  }

  return button;
};

const handleButtonClick = async (
  button: HTMLButtonElement,
  savedPostKeys: Set<string>
) => {
  const currentState =
    (button.dataset.state as ButtonState | undefined) ?? "idle";
  if (!shouldAllowClick(currentState)) {
    return;
  }

  const permalink = button.dataset.permalink;
  const postKey = button.dataset.postKey;
  const platform = button.dataset.platform as Platform | undefined;
  if (!(permalink && postKey && platform)) {
    setButtonState(button, "error", "Invalid post data");
    window.setTimeout(() => setButtonState(button, "idle"), 2500);
    return;
  }

  setButtonState(button, "saving");

  try {
    const response = await sendRuntimeMessage<TeakSaveResponse>({
      type: MESSAGE_TYPES.SAVE_POST,
      payload: {
        platform,
        permalink,
        postKey,
      },
    });

    const nextState = mapSaveResponseToButtonState(response);
    if (nextState === "saved") {
      savedPostKeys.add(postKey);
      setButtonState(button, "saved");
      return;
    }

    if (nextState === "duplicate") {
      savedPostKeys.add(postKey);
      setButtonState(button, "duplicate");
      return;
    }

    if (response.status === "unauthenticated") {
      setButtonState(button, "error", "Sign in to Teak to save");
      window.setTimeout(() => setButtonState(button, "idle"), 2500);
      return;
    }

    const errorMessage =
      response.status === "error" ? response.message : "Failed to save post";
    setButtonState(button, "error", errorMessage);
    window.setTimeout(() => setButtonState(button, "idle"), 2500);
  } catch (error) {
    const message =
      error instanceof Error ? error.message : "Failed to save post";
    setButtonState(button, "error", message);
    window.setTimeout(() => setButtonState(button, "idle"), 2500);
  }
};

function bindHackerNewsHoverState(postElement: HTMLElement) {
  if (postElement.dataset.teakHoverScopeBound === "1") {
    return;
  }

  const titleRow = postElement.closest("tr.athing");
  const subtextRow = titleRow?.nextElementSibling;
  if (!(titleRow instanceof HTMLElement)) {
    return;
  }

  const hoverScope: HTMLElement[] = [titleRow];
  if (subtextRow instanceof HTMLElement) {
    hoverScope.push(subtextRow);
  }

  const showButton = () => {
    postElement.classList.add("teak-hn-hovering");
  };

  const hideButton = (nextTarget?: EventTarget | null) => {
    if (
      nextTarget instanceof Node &&
      hoverScope.some((element) => element.contains(nextTarget))
    ) {
      return;
    }

    postElement.classList.remove("teak-hn-hovering");
  };

  for (const element of hoverScope) {
    element.addEventListener("mouseenter", showButton);
    element.addEventListener("mouseleave", (event) => {
      hideButton(event.relatedTarget);
    });
    element.addEventListener("focusin", showButton);
    element.addEventListener("focusout", (event) => {
      hideButton(event.relatedTarget);
    });
  }

  postElement.dataset.teakHoverScopeBound = "1";
}

const scanAndInjectButtons = ({
  platformBinding,
  savedPostKeys,
}: {
  platformBinding: PlatformBinding;
  savedPostKeys: Set<string>;
}) => {
  const posts = platformBinding.findPosts(document);
  for (const postElement of posts) {
    if (postElement.dataset.teakSaveBound === "1") {
      continue;
    }

    const extractedPost = platformBinding.extractPost(postElement);
    if (!extractedPost) {
      continue;
    }

    platformBinding.bindHoverScope?.(postElement);

    if (
      getInlineSaveButtonVariant(extractedPost.platform) === "overlay" &&
      getComputedStyle(postElement).position === "static"
    ) {
      postElement.dataset.teakPositionManaged = "1";
      postElement.dataset.teakOriginalPosition = postElement.style.position;
      postElement.style.position = "relative";
    }

    const button = createSaveButton(extractedPost, savedPostKeys);
    button.addEventListener("click", () => {
      void handleButtonClick(button, savedPostKeys);
    });

    const didMount =
      platformBinding.mountButton?.(postElement, button) ??
      (postElement.append(button), true);
    if (!didMount) {
      continue;
    }

    postElement.dataset.teakSaveBound = "1";
  }
};

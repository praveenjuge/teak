...
- API: your Convex site URL, for example `https://<deployment>.convex.site/v1`
- MCP Endpoint: your Convex site URL plus `/mcp`
...
Convex HTTP actions expose the REST API at `/v1` and the MCP endpoint at `/mcp`.
Use the `CONVEX_SITE_URL` printed by `bunx convex dev` or shown in the Convex
Dashboard.

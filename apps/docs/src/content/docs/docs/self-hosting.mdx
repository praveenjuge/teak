...
The public REST API and MCP server are served by Convex HTTP actions on your
Convex `.site` domain. Set `NEXT_PUBLIC_CONVEX_SITE_URL` in `apps/web` to that
same URL, and use `TEAK_DEV_API_URL` only when you need a local override for
development clients.

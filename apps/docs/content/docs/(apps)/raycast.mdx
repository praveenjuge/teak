...
<Steps>
  <Step title="Install the extension">
    <Frame caption="Install Teak from the Raycast Store.">
      <a href="https://www.raycast.com/praveenjuge/teak-raycast">
        <img
          alt="Install the Teak Raycast extension"
          height="104"
          src="https://www.raycast.com/praveenjuge/teak-raycast/install_button@2x.png?v=1.1"
          width="320"
        />
      </a>
    </Frame>
  ...
  <Step title="Sign in with your browser">
    Run any Teak command and choose **Sign in with Browser**. Your browser opens to authorize Teak. Approve access on the consent screen, then Raycast finishes sign-in — no key to copy.
  </Step>
</Steps>

### Using an API key (optional)

Prefer an API key? Paste one from Teak Settings into the extension preferences; a configured key takes precedence over browser sign-in.
...
- Browser sign-in connects Raycast without copying a key. Use **Sign Out** from any command's action panel to revoke this installation (if Teak is unreachable, credentials stay saved so you can retry), or **Settings → Security → Connections** to disconnect every installation.
- API keys are managed under **Settings → Security → API keys** — see the [API guide](/docs/api) for key limits and rotation.

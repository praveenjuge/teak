...
`teak logout` revokes this installation's browser sign-in (if Teak is unreachable, credentials stay saved so you can retry). To disconnect every CLI installation at once, use **Settings → Security → Connections**. API keys are unaffected.

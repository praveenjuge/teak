...
`CLERK_JWT_ISSUER_DOMAIN` must match your Clerk JWT issuer domain. `SITE_URL` must match your Next.js origin. `CONVEX_SITE_URL` is the `.site` domain assigned by Convex—copy it from the dashboard.

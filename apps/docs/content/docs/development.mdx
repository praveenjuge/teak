...
- **Context Menu**: Right-click to save selected text, links, and images
- **Auto-Save**: Click extension icon to save current tab with metadata
- **Authentication**: Uses same Clerk authentication as web/mobile apps

### Publishing

```bash
bun run package:extension  # Creates production ZIP for Chrome Web Store
```

## Deployment

### Convex Backend
- Functions deploy automatically during development
- Production deployment via Convex dashboard
...
### Mobile App
- Build with Expo EAS for app stores
- Configure push notifications and native features
...
This guide will help new developers get up and running with the Teak project quickly.
...
- Use `bun run db:studio` to visually inspect and modify database data
- The project uses TypeScript throughout - leverage the type system
- API testing collection is available in the `/postman` directory
- Each workspace has its own `package.json` for workspace-specific dependencies
- Docker containers handle database setup automatically
- Check the `/scripts` directory for additional utility scripts

## 🔍 Troubleshooting

If you encounter issues:

1. **Database connection problems**: Run `bun run db:status` to check connection
2. **Port conflicts**: Check if ports 3000, 3001, or 5432 are already in use
3. **Docker issues**: Try `bun run docker:dev:clean` to reset containers
4. **Dependency issues**: Run `bun run install:all` to reinstall dependencies
5. **Environment issues**: Verify your `.env` file is properly configured

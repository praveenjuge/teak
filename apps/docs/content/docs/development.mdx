...
│   │   │   ├── aiBackfill.ts # AI backfill workflow
│   │   │   ├── cardCleanup.ts # Data cleanup workflow
│   │   │   ├── cardProcessing.ts # Main card processing
│   │   │   ├── linkEnrichment.ts # Link enrichment
│   │   │   └── linkMetadata.ts # Link metadata workflow
│   │   ├── http.ts       # HTTP route handlers
│   │   ├── migrations.ts # Database migrations
│   │   ├── admin.ts      # Admin functions
│   │   ├── billing.ts    # Polar billing integration
...
- **AI Processing**: OpenAI API for content analysis and metadata generation
- **Screenshot Service**: Cloudflare Browser Rendering API for link previews
...
bun run test:mobile      # Run mobile tests (if available)
bun run test:extension   # Run extension tests (if available)
...
- **Steps**: Each stage in `backend/convex/workflows/steps/` contains mutations and processing logic
  - **Categorization**: Content categorization with provider-specific handling (Amazon, GitHub, Goodreads, IMDb)
  - **Link Metadata**: Link metadata extraction using Cloudflare Browser Rendering API
  - **Renderables**: Thumbnail and visual asset generation for media files
  - **Screenshot**: Screenshot capture for link previews
...
- **AI Tasks**: Background AI processing in `backend/convex/tasks/ai/` for content analysis and metadata generation

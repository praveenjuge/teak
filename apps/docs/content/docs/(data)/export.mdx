...
<Steps>
  <Step title="Open Export">
    Open Teak on web, or the desktop app via **Teak → Settings…**, then find **Export Data** and choose **Manage**.
  </Step>
  <Step title="Start export">
    Choose **Start export**, then keep working while Teak prepares the archive in the background.
  </Step>
  <Step title="Download">
...
Each export is a ZIP with your active cards (`cards.json`), their original files (`files/`), and a manifest. Importing the archive adds the exported cards and any included files to your library.

## What Is Not Included

Exports focus on portable library content: trash, AI enrichment, generated previews, and account data are not included. If an original file cannot be read, its card is still included without the file.

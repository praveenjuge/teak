...
`https://teakvault.com/mcp` is Teak's **card** MCP (save, search, sync). It is not a documentation search server. For human and agent-readable docs, start at [`/llms.txt`](https://teakvault.com/llms.txt) or any page's `.md` URL.
:::
...
<Prompt
  ...
  actions={["copy", "cursor"]}
>
  Run: claude mcp add --transport http teak https://teakvault.com/mcp
  Then open /mcp in Claude, complete browser sign-in, and list my favorite cards.
</Prompt>

...
<Callout type="tip" title="Product MCP vs docs">
`https://teakvault.com/mcp` is Teak's **card** MCP (save, search, sync). It is
not a documentation search server. For human and agent-readable docs, start at
`/llms.txt` or any page's `.md` URL.
</Callout>

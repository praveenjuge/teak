...
Image card responses include three intentionally different URLs: `thumbnailUrl`
is the optimized grid rendition, `detailUrl` is the optimized full-card
rendition, and `fileUrl` remains the signed original used for downloads. These
URLs are temporary bearer credentials; clients should consume them as returned
instead of storing or modifying them.

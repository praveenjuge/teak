...
File cards include a `previewUrl` in responses. For large images this points
to a downscaled WebP derivative so clients can show a fast modal/image view
without downloading the full original; `thumbnailUrl` remains the small grid
thumbnail and `fileUrl` the original.

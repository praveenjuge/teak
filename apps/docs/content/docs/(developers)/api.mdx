...
:::tip[Prefer the apex API URL]
New integrations should use `https://teakvault.com/api/v1`. The legacy `api.teakvault.com` host keeps working, so existing keys and scripts need no changes.
:::

<Diff
  old={`curl https://api.teakvault.com/v1/cards \\
  --header "Authorization: Bearer $TEAK_API_KEY"`}
  new={`curl https://teakvault.com/api/v1/cards \\
  --header "Authorization: Bearer $TEAK_API_KEY"`}
/>

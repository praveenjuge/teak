...
## Deprecated search routes

`GET /v1/cards/search` and `GET /v1/cards/favorites` are deprecated and return
`Deprecation`, `Link`, and `Warning` headers pointing at the replacement. Both
routes still work, and no removal date is set yet.

Use `GET /v1/cards` instead. It supports the same `q`, `type`, `tag`, `sort`,
`createdAfter`, `createdBefore`, `limit`, and `cursor` parameters, plus
`favorited` and `include`:

- Search: `GET /v1/cards?q=design&include=content,metadata`
- Favorites: `GET /v1/cards?favorited=true&include=content,metadata`

The `include=content,metadata` pair returns the same full-card fields the
deprecated routes return. The list response is paginated (`items` plus
`pageInfo`) instead of returning a `total` count.

The CLI, SDK, MCP tools, and the Raycast extension switch to the list
endpoint for search and favorites with the upcoming Teak 1.0.70 release, so
upgrading once it ships is the migration for most integrations.

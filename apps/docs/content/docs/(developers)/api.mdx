...
Clients released with Teak 1.0.70 or later (CLI, SDK, MCP tools, and the
Raycast extension) already use the list endpoint for search and favorites, so
upgrading is the migration for most integrations.

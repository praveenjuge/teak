...
- **Optional Features**:
  - `R2_*`: Create a private Cloudflare R2 bucket for the Files Worker's `BUCKET` binding. The backend needs no R2 credentials: all file bytes flow through the worker.

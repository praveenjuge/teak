...
Only `BETTER_AUTH_SECRET` and `TEAK_ADMIN_EMAIL` are required to boot; add the rest for the features you want. See [Where to get the values](#where-to-get-the-values) for sources.

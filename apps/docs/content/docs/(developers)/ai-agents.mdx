...
:::note[Docs stay static]
Teak's public docs ship as a static site. There is no in-page Ask AI assistant
and no documentation-search MCP on this host — `/mcp` is reserved for Teak's
product card MCP. Use `llms.txt`, page `.md` mirrors, and the OpenAPI reference
instead.
:::

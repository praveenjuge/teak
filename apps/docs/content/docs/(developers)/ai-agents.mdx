...
:::tip[Two different MCP surfaces]
`https://teakvault.com/mcp` is Teak's **card** MCP (save, search, sync cards). It is not a docs-search server. Prefer `llms.txt`, page `.md` URLs, and the OpenAPI reference when an agent needs product documentation.
:::

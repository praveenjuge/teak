...
2. Start Convex dev and set up Clerk defaults:
...
npx convex env set AUTH_SECRET_PEPPER $(openssl rand -base64 32)
...
AUTH_SECRET_PEPPER=your-secret               # API key and rate-limit hashing
...
EXPO_PUBLIC_CLERK_PUBLISHABLE_KEY=pk_test_...     # Mobile only
TEAK_DEV_APP_URL=http://app.teak.localhost:1355   # Optional local override
...
- **Clerk**:
  - `NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY` / `EXPO_PUBLIC_CLERK_PUBLISHABLE_KEY`: From your Clerk dashboard for web and mobile clients.
  - `CLERK_SECRET_KEY`: Server-side key for the Next.js app and E2E setup script.
  - `CLERK_JWT_ISSUER_DOMAIN`: Clerk issuer domain used by Convex auth.
  - `CLERK_WEBHOOK_SECRET`: Signing secret for the Convex Clerk webhook endpoint.
  - `AUTH_SECRET_PEPPER`: Random string used to hash API keys and rate-limit keys.

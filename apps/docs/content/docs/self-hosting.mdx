...
Copy the values from your Convex dashboard (or the `convex dev` output) into these files:
...
**`apps/extension/.env.local`** (Extension)
...
The web app ships with Sentry pre-configured for the hosted Teak service. If you are self-hosting and want your own error tracking, replace the DSN in `apps/web/sentry.server.config.ts`, `apps/web/sentry.edge.config.ts`, and `apps/web/src/instrumentation-client.ts` with your own [Sentry DSN](https://docs.sentry.io/product/sentry-basics/dsn-explainer/). To disable Sentry entirely, remove the `@sentry/nextjs` initialisation calls from those three files and uninstall the package.
